/**
 * Autonomous order fulfilment for Recast — runs in GitHub Actions on every new issue.
 * Verifies the buyer's USDC payment on Base, signs a Pro license, posts it into the
 * issue, records the tx, and closes. No human in the loop. Signing key is an encrypted
 * Actions secret (LICENSE_PRIVATE_JWK), never in the repo.
 */
...
if (!receipt) { comment("That transaction isn't confirmed yet. Wait for ~1 confirmation on Base, then **edit this issue** to re-run."); process.exit(0); }
...
comment(["✅ **Payment verified — your Recast Pro license:**", "", "Open Recast → 🔓 Pro → *Have a key?* → paste:", "", "```", license, "```", "", "Unlimited, lifetime. Your key, your data. Thank you! 🔒"].join("\n"));
